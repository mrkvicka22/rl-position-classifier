...
def apply_normal_randomization(batch, use_2d_map=False):
    ...
    batch[mask, ndims:2 * ndims] = (
            (1 - 2 * np.random.random(size=(batch_size // 2, ndims))) * rng_pos_mul + rng_pos_off)  # (x, y, z) range.
...
def train(model, dataset: DatasetClass, epochs: int, batch_size: int, optimiser, loss_fn, randomisation=None,
          augment_flip=False, use_2d_map=False):
    ...
    total_steps = 0
    init_features, init_labels = get_state_batch(dataset, batch_size, 'train', randomisation=randomisation,
                                                 augment_flip=False, use_2d_map=use_2d_map)
    ...
    for epoch in epoch_iterator:
        ...
        while epoch_steps < epoch_length:
            train_features, train_labels = get_state_batch(dataset, batch_size, 'train', randomisation=randomisation,
                                                           augment_flip=augment_flip, use_2d_map=use_2d_map)
            inputs = torch.tensor(train_features.astype(np.float32))
            labels = torch.tensor(train_labels.astype(np.float32)).view(
                (batch_size, 1))  # BCELoss requires strict size for labels
            training_loss = train_step(model, optimiser, loss_fn, inputs, labels)

            epoch_steps += batch_size
            total_steps += batch_size
            if epoch_steps % 100_000 == 0:
                print('Epoch {}{}, steps: {}, ({:.2f}%)'.format(epoch + 1, epoch_total, total_steps,
                                                                epoch_steps / epoch_length * 100))
                # TODO: Use "last_log_steps" instead, so that batch_size doesn't need to be a factor of 100_000
                wandb.log(
                    {'train_loss': training_loss, 'learning_rate': optimiser.param_groups[0]["lr"], 'epoch': epoch,
                     'steps': total_steps}, step=total_steps)

        model.eval()
        with torch.no_grad():
            validation_batch_size = 50_000
            # Validate
            val_features, val_labels = get_state_batch(dataset, validation_batch_size, 'validation',
                                                       randomisation=randomisation, augment_flip=False,
                                                       use_2d_map=use_2d_map)
            val_inputs = torch.tensor(val_features.astype(np.float32))
            val_labels = torch.tensor(val_labels.astype(np.float32)).view(
                (validation_batch_size, 1))  # BCELoss requires strict size for labels
            ...
            train_features, train_labels = get_state_batch(dataset, validation_batch_size, 'train',
                                                           randomisation=randomisation, augment_flip=False,
                                                           use_2d_map=use_2d_map)
            ...
            wandb.log({'val_loss': val_loss, 'val_acc': val_acc, 'train_loss': train_loss,
                       'learning_rate': optimiser.param_groups[0]["lr"], 'epoch': epoch, 'steps': total_steps},
                      step=total_steps)
            model.train()
            ...
            try:
                ...
                with torch.no_grad():
                    ...
                    for _ in range(test_passes):
                        test_features, test_labels = get_state_batch(dataset, test_batch_size, 'test',
                                                                     randomisation=randomisation, augment_flip=False,
                                                                     use_2d_map=use_2d_map)
                        test_inputs = torch.tensor(test_features.astype(np.float32))
                        test_labels = torch.tensor(test_labels.astype(np.float32)).view(
                            (test_batch_size, 1))  # BCELoss requires strict size for labels
                ...
                # Render
                create_animation_from_model(best_model_path, render_path, player_count=dataset.player_count,
                                            image_size=0.5, use_2d_map=use_2d_map, )
    ...
    with torch.no_grad():
        ...
        test_loss, test_acc = 0, 0
        for _ in range(test_passes):
            test_features, test_labels = get_state_batch(dataset, test_batch_size, 'test', randomisation=randomisation,
                                                         augment_flip=False, use_2d_map=use_2d_map)
            test_inputs = torch.tensor(test_features.astype(np.float32))
            test_labels = torch.tensor(test_labels.astype(np.float32)).view(
                (test_batch_size, 1))  # BCELoss requires strict size for labels
...
if __name__ == '__main__':
    def __main__():
        ...
        parser.add_argument('--dataset', type=str, default='ssl_2v2', choices=[d.table for d in available_datasets],
                            help='Dataset to use')
        ...
        parser.add_argument('--randomisation', type=str, default='normal', choices=['normal', 'shuffle', 'none'],
                            help='Random position strategy')
        ...
        parser.add_argument('--dropout', type=float, default=None,
                            help='Dropout percentage, defaults to None which disables dropout. Range 0-1.')
        ...
        parser.add_argument('--dropout-layers', type=int, default=None,
                            help='Number of dropout layers, defaults to number of hidden layers, ignored if dropout percentage is not specified')
        ...
        wandb.init(project="rl-position-classifier", entity='mrkvicka02', config=args)
        args = wandb.config
        ...
        model = create_model(dataset.player_count, 2 if use_2d_map else 3, hidden_layers=hidden_layers,
                             hidden_units=hidden_units, dropout=dropout, dropout_layers=dropout_layers)
        wandb.watch(model)
        ...
        train(
            model=model
            , dataset=dataset
            , epochs=1  # (total cycles through the dataset)
